...
def generate_transactions(accounts_dict, num_transactions=50000):
    ...
    for i in range(num_transactions):
        # Select source and destination accounts (they can be the same)
        ...
        from_risk = from_account['risk_score']
        to_risk = to_account['risk_score']
...
def generate_alerts(transactions, accounts_dict):
    ...
    if mule_transactions:
        ...
        alert = {
            ...
            'type': 'mule_activity',
            'severity': 'high',
            'title': f'Multiple Mule Accounts Detected ({len(mule_transactions)} transactions)',
            'description': f'{unique_accounts} unique accounts involved in suspicious transactions totaling ${total_mule_amount:,.2f}',
            'accounts': list(set([t['from'] for t in mule_transactions] + [t['to'] for t in mule_transactions])),
    ...
    if high_value_txns:
        ...
        alert = {
            ...
            'accounts': list(set([t['from'] for t in high_value_txns] + [t['to'] for t in high_value_txns])),
            'amount': total_amount,
    ...
    if suspicious_txns:
        alert = {
            ...
            'accounts': list(set([t['from'] for t in suspicious_txns] + [t['to'] for t in suspicious_txns])),
    ...
    if high_risk_accounts:
        alert = {
            ...
            'accounts': [a['account_id'] for a in high_risk_accounts],
...
def generate_network_transfers(accounts_dict, max_transfers_per_account=10):
    ...
    for account_id in account_ids:
        ...
        for _ in range(num_transfers):
            ...
            from_risk = account['risk_score']
            to_risk = recipient['risk_score']
            amount_risk = min(amount / 10000, 1.0)
            
            transaction_risk = (from_risk * 0.5 + to_risk * 0.3 + amount_risk * 0.2)
...
def generate_all_synthetic_data():
    ...
    # Write to file
    with open('public/synthetic_dataset.json', 'w') as f:
        json.dump(synthetic_data, f, indent=2)
    
    size_mb = os.path.getsize('public/synthetic_dataset.json') / (1024*1024)
