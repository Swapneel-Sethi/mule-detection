...
**Generated:** 2026-08-25T12:55:57.397Z  
...
|-----------------------|---------------:|----------------:|
| **Actual MULE**       | TP = 56 | FN = 44 |
| **Actual LEGIT**      | FP = 36 | TN = 264 |

| Metric (mule class) | Value |
|---|---:|
| Accuracy | 80.00% |
| Precision | 0.6087 (60.87%) |
| Recall (mule detection rate) | 0.5600 (56.00%) |
| F1 (mule) | 0.5833 |
| AUC — calibrated_score | 0.8072 |
| AUC — risk_score (0–100) | 0.8072 |
...
|---|---:|---:|---:|
| Planted mules | 100 | 0.5713 | 57.13 |
| Legit accounts | 300 | 0.4227 | 42.27 |
...
|---|---:|---:|---:|---:|
| fan_in | 25 | 18 | 72.00% | 0.6329 |
| fan_out | 25 | 16 | 64.00% | 0.5810 |
| pass_through | 25 | 16 | 64.00% | 0.5966 |
| circular | 25 | 6 | 24.00% | 0.4748 |
...
Rank discrimination (AUC on the decision variable, `calibrated_score`) is **0.807** — good; clearly above the 0.5 chance line.

At the engine's own operating point, recall of planted mules is **56.00%** at precision **0.609** (accuracy 80.00% vs. majority-class baseline 75.00%).
...
The `critical` risk band (calibrated ≥ 0.671) contains 14 accounts — 14 mules vs 0 legit (band precision 100.00%) — while the `high` band is empty: the Platt calibration's near-step shape makes the bands unusable for triage.
