...
**Generated:** 2026-08-25T12:20:22.882Z  
...
| **Actual LEGIT**      | FP = 141 | TN = 159 |

| Metric (mule class) | Value |
|---|---:|
| Accuracy | 55.00% |
| Precision | 0.3020 (30.20%) |
| Recall (mule detection rate) | 0.6100 (61.00%) |
| F1 (mule) | 0.4040 |
| AUC — calibrated_score | 0.6705 |
| AUC — risk_score (0–100) | 0.6705 |
...
|---|---:|---:|---:|
| Planted mules | 100 | 0.5649 | 56.49 |
| Legit accounts | 300 | 0.3973 | 39.73 |
...
|---|---:|---:|---:|---:|
| fan_in | 25 | 13 | 52.00% | 0.5126 |
| fan_out | 25 | 16 | 64.00% | 0.6200 |
| pass_through | 25 | 16 | 64.00% | 0.6077 |
| circular | 25 | 16 | 64.00% | 0.5192 |
...
Rank discrimination (AUC on the decision variable, `calibrated_score`) is **0.670** — barely above chance (0.5); weak signal.

At the engine's own operating point, recall of planted mules is **61.00%** at precision **0.302** (accuracy 55.00% vs. majority-class baseline 75.00%).
...
The `critical` risk band (calibrated ≥ 0.671) contains 193 accounts — 59 mules vs 134 legit (band precision 30.57%) — while the `high` band is empty: the Platt calibration's near-step shape makes the bands unusable for triage.
